...
export default function GuessGamePlay() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        else if (d.game_status === 'playing') {
          setPhase('playing');
          startTimer(d.guess_time_seconds || 10);
        } else if (d.game_status === 'finished') {
          setPhase('finished');
          setResult(d);
        }
      } catch {
  ...
  const handleSubmitSecretComputer = async () => {
    ...
    setError('');
    try {
      const res = await axios.post(`${API}/guess-game/${roomKey}/secret`, {
        player_id: playerId,
        secret_number: num,
      });
      setGameData(res.data);
      setPhase('playing');
      startTimer(res.data.guess_time_seconds || 10);
    } catch (e) {
  ...
  const handleSubmitGuessComputer = async () => {
    ...
    setGuessInput('');
    try {
      const res = await axios.post(`${API}/guess-game/${roomKey}/guess`, {
        player_id: playerId,
        guess_number: num,
      });
      const d = res.data;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const ws = new WebSocket(
        `${WS_BASE}/ws/guess-game/${roomKey}?player_id=${playerId}`,
      );
      let opened = false;
      ...
      ws.onerror = () => {
        // onclose handles retries + final error state
      };

      ws.onclose = () => {
        if (cancelled) return;

        // Only auto-retry if connection couldn't be established
        if (!opened && attempts < MAX_RETRIES) {
          attempts += 1;
        ...
        // If socket never connected, show a clear actionable message
        if (!connectedOnce) {
          setError(
            "Couldn't connect to realtime game server. Please retry in a few seconds (backend may be waking up) or refresh.",
          );
          setPhase('error');
        }
      };
  ...
  }, [roomKey, isMultiplayer, playerId]);
      ...
      case 'game_over':
        ...
        setResult({
          ...
          message:
            msg.winner === playerId
              ? 'You guessed it!'
              : `${msg.winner_name} guessed your number!`,
        });
        break;
  ...
  const handleGameState = (state) => {
    ...
    if (
      state.player2 &&
      state.player2.player_id !== playerId &&
      state.player2.name
    ) {
    ...
    if (
      state.player1 &&
      state.player1.player_id !== playerId &&
      state.player1.name
    ) {
  ...
  // Unified handlers
  const onSubmitSecret = isMultiplayer
    ? handleSubmitSecretMultiplayer
    : handleSubmitSecretComputer;
  const onSubmitGuess = isMultiplayer
    ? handleSubmitGuessMultiplayer
    : handleSubmitGuessComputer;
  const onKey = (e, fn) => {
    if (e.key === 'Enter') fn();
  };
    ...
    return (
      <Shell>
        <div style={{ textAlign: 'center', padding: '80px 24px' }}>
          ...
          <button onClick={() => navigate('/guess-game')} style={btnPrimary}>
            New Game
          </button>
        </div>
      </Shell>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '28px 24px' }}>
        ...
        >
          ...
          >
            ...
            {isMultiplayer && (
              <span style={{ color: '#f97316', marginLeft: 8 }}>2P</span>
            )}
          </div>
          {phase === 'playing' && (
            <div
        ...
        {phase === 'select_range' && (
          <div>
            <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>
              {isHost
                ? 'Choose the range'
                : 'Waiting for host to pick range...'}
            </h2>
            {opponentName && (
              ...
              <>
                ...
                <button
                  ...
                  style={{
                    ...btnPrimary,
                    width: '100%',
                    opacity: submittingSecret ? 0.5 : 1,
                  }}
                >
          ...
          <div>
            ...
            {isMultiplayer && !isMyTurn ? (
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                >
                  Your turn — guess {isMultiplayer ? 'their' : "the computer's"}{' '}
                  number
                </div>
        ...
        {phase === 'finished' && result && (
          <div>
            ...
            >
              ...
              <h2
                style={{
                  ...
                  fontWeight: 700,
                  marginBottom: 8,
                  color: result.you_won ? '#4ade80' : '#f97316',
                }}
              >
                {result.you_won
                  ? 'You Won!'
                  : `${result.winner_name || 'Opponent'} Won!`}
              </h2>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
              ...
              <SecretCard
                label={isMultiplayer ? 'THEIR SECRET' : "COMPUTER'S SECRET"}
                value={result.player2_secret}
              />
            </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span
          style={{
            ...
            fontWeight: 700,
            fontFamily: "'Space Mono', monospace",
            color: g.result === 'correct' ? '#4ade80' : '#e8e8e8',
          }}
        >
          {g.number}
        </span>
